...
function findElementGreaterThan(arr, num) {
  ...
  const firstElem = arr.find(number => number > num)
...
function findIndexByProperty(arr, name) {
  ...
  const findIndex = arr.findIndex((element) => element.name === name)
  // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
  return findIndex >= 0 ? findIndex: "Objeck not found" 
}
...
function findLastEvenNumber(arr) {
  ...
  const numEqual = arr.findLast((elem) => elem % 2 === 0)
  // Повертаємо знайдене число або 'Парних чисел не знайдено', якщо таке число не знайдено.
  return numEqual >= 0 ? numEqual: "Парних чисел не знайдено";
}
...
function getPositiveArrayValues(arr) {
  ...
  let posValues = obj.filter((elem) => elem > 0)
...
function sumAndPrint(arr) {
  ...
  let sum = arr.reduce((acumulator, currentValue) => acumulator + currentValue);   
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let arrFilter = arrMap.filter((e) => e >= threshold)
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let sliceArr = arr.slice(n + 1, arr.length);  
  // За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.
  let sum = sliceArr.reduceRight((acumulator, currentValue) => acumulator + currentValue) 
  // За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
  sliceArr.forEach((element) => console.log(element))
...
function expandArrayByN(arr, n) {
  ...
  let flatArr = arr.flatMap((el) => el === el?[el, el] : n)
  // Повертаємо розширений масив
  return flatArr
}

console.log("Завдання: 14 ==============================");
console.log(expandArrayByN([1, 2, 3, 3], 3)); // Виведе [1, 1, 1, 2, 2, 2, 3, 3, 3]
...
function findLongestWord(arr) {
  ...
  let arrLength = arr.reduce((acumulator, currentValue) => {
    if (acumulator.length > currentValue.length) {
      return acumulator
    }
...
function findDuplicateElements(arr) {
  ...
  let arrFilter = arr.filter((element, index) => arr.indexOf(element) !== index)  
...
function capitalizeWords(arr) {
  ...
  let capitalizedArray = arr.map((element) => element.charAt(0).toUpperCase() + element.slice(1))
...
function calculateTotalPrice(arr) {
  ...
  let sum = arr.reduce((acum, currentValue, i) => acum + currentValue.price, 0)
...
function findLastNegativeNumberIndex(arr) {
  ...
  arr = arr.findLastIndex((elem, index) => elem < index)
